...
void ImGui::ColorPicker(const char* name, float color[3]) noexcept
{
    ...
    bool popup = ImGui::ColorButton(name, ImVec4{ color[0], color[1], color[2], 1.0f }, picker_flags); 
    ImGui::SameLine();
    ImGui::Text(name);
    if (popup) ImGui::OpenPopup(name);
    if (ImGui::BeginPopup(name)) {
        ImGui::PushItemWidth(150.f);
        ImGui::ColorPicker3(buffer, color, ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_NoSidePreview);
        ImGui::EndPopup();
    }
}
...
IMGUI_API void ImGui::hotkey(const char* label, KeyBind& key, float samelineOffset, const ImVec2& size) noexcept
{
